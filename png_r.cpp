...
void read_png_file(const char* filename, const char* output_filename) {
    ...
    // Use a string stream to store the output
    std::ostringstream output_stream;
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width * 3; x += 3) {
            output_stream << "14,0,0," 
                          << (new_height - 1 - y) << "," << x / 3 << ","
                          << (int)resized_row_pointers[y][x] << "+" 
                          << (int)resized_row_pointers[y][x + 1] << "+" 
                          << (int)resized_row_pointers[y][x + 2] << "+1+0";
    ...
    // Write the output to the clipboard using xclip (Linux) or pbcopy (macOS)
    FILE* clipboard = popen("xclip -selection clipboard", "w"); // Use "pbcopy" for macOS
    if (clipboard) {
        fputs(output_stream.str().c_str(), clipboard);
        pclose(clipboard);
    } else {
        std::cerr << "Error: Unable to access clipboard." << std::endl;
    }
